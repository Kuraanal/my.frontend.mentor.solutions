...
themeSlider.addEventListener("change", function() {
    document.body.dataset.theme = themeSlider.value;
}, false);
...
keys.forEach(key => {
    key.addEventListener("click", function(){
        ...
        if(key.dataset.type == "modifier"){
            ...
            reset();
        else if(key.dataset.value == "delete")
            deleteOperand();
...
function operate(){ // appuis sur Equal
...
function updateScreen(){
    ...
    smlScreen.textContent = parseFloat(previousOperand).toLocaleString() + operator;
}